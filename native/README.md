...
| `ctt-radio-touch` | The precise 1200-baud touch that resets an ATmega32U4 Feather into its Caterina bootloader — the one native step of flashing. Orchestration (channel resolve, by-path, mode detect) + `avrdude` live in `program-radio.sh`. | the radio's serial port | the board reset into its bootloader |
