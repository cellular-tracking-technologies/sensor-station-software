...
ProvisionResult TelitLE910Q1::provision(bool dry_run) {
  ...
  std::string u = at_.cmd(at::kUsbcfgQuery, 3000);
  ...
  if (mode != 1) {
    ...
    std::string w = at_.cmd(at::kUsbcfgEcm, 5000);
  ...
  std::string e = at_.cmd(at::kEcmQuery, 3000);
  ...
  std::fprintf(stderr, "ctt-modem-provision: binding ECM to PDP context 1 "
                       "(AT#ECM=1,0)\n");
  std::string w = at_.cmd(at::kEcmBind, 5000);
  if (w.find("OK") == std::string::npos) {
