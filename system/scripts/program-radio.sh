#!/bin/bash
# Flash one radio MCU (Adafruit Feather 32u4). This script IS the flasher: it does all
# the orchestration and hands the two hardware steps to dedicated tools —
#   - ctt-radio-touch : the precise 1200-baud touch that resets an app board into its
#                       Caterina bootloader (the one step that must be native)
...
if [ "$pid" = "800c" ] || [ -z "$pid" ]; then
  ...
  ctt-radio-touch "$bypath" || { echo "program-radio: touch failed on $bypath" >&2; exit 1; }
