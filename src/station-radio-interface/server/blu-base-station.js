...
class BluStation {
   ...
   */
  startBluRadios(path) {
    this.blu_receiver = new BluReceiverManager({
      path: path,
    })
    ...
    this.blu_receiver.startUpFlashLogo()

    setInterval(() => {
      this.blu_receiver.getBluVersion(1)
      this.blu_receiver.getBluVersion(2)
      this.blu_receiver.getBluVersion(3)
      this.blu_receiver.getBluVersion(4)
    }, 10000)
    ...
    const radios_start = Promise.all(this.blu_receiver.blu_radios
      .map((radio) => {
        ...
        this.blu_receiver.radioOn(radio, radio.poll_interval)
      })).then((values) => {
  ...
  stopBluRadios(path) {
    if (path !== undefined) {
      ...
      const radios_exit = Promise.all(this.blu_receivers.blu_radios
        .map((radio) => {
          this.blu_receiver.radioOff(radio)
