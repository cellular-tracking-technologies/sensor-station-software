    ...
    this.sensor_socket_server.on('cmd', (cmd) => {
      switch (cmd.cmd) {
        ...
        case ('toggle_blu'):
          ...
          if (cmd.data.type === 'blu_on') {
            ...
            const radios_on = Object.keys(this.blu_radios).map(radio => {
              ...
              blu_reader.updateConfig(this.config.default_config)
              blu_reader.radioOn(Number(radio), cmd.data.poll_interval)
            })
        ...
        case ('reboot_blu_radio'):
          ...
          setTimeout(() => {
            blu_reader.rebootBluReceiver(Number(cmd.data.channel), this.poll_data.poll_interval)
          }, 10000)
          break
        case ('change_poll'):
          ...
          if (Object.keys(this.config.default_config.blu_radios).includes(poll_key.toString())) {
            ...
            blu_reader.stopDetections(Number(cmd.data.channel))
            blu_reader.setBluConfig(Number(cmd.data.channel), { scan: 1, rx_blink: 1, })
            blu_reader.getDetections(Number(cmd.data.channel), this.poll_interval)
          }
          break
        case ('update-blu-firmware'):
          ...
          blu_reader.updateBluFirmware(radio_num, this.firmware, poll_interval)
    ...
    chokidar.watch('../../../../../../dev/serial/by-path')
      .on('add', path => {
        ...
        if (!path.includes('0:1.2.') && path.includes('-port0')) {
          ...
          // this.blu_reader.path = path
          this.blu_receiver.push(this.blu_reader)
          blu_reader.path = path
          // this.blu_receiver.push(blu_reader)
          console.log('blu receiver', this.blu_receiver)
          // this.startBluRadios(path)
          this.startBluRadios(blu_reader)
          // this.startBluRadios(this.blu_reader)
          // blu_reader.destroy_receiver()
          // this.blu_reader.destroy_receiver()
          // blu_reader = undefined
          // console.log('directory watcher blu reader after destruction', this.blu_reader)
      ...
      .on('unlink', path => {
        // this.blu_reader.emit('close', () => {
        // blu_reader.emit('close', () => {
        //   console.log('lets see if this destroys the instance')
        // })
        console.log('file', path, 'has been removed')
        blu_reader.destroy_receiver()
        console.log('blu receiver array before undefined', blu_reader)

        // Object.keys(this.blu_reader.blu_radios).forEach((radio) => {
        //   this.blu_reader.destroy(radio)
        // })
        ...
        // this.blu_reader.destroy_receiver()
        // console.log('blu receiver array before undefined', this.blu_reader)
 ...
 */
  ...
  startBluRadios(blu_reader) {
    ...
    blu_reader.on('close', () => {
      ...
      console.log('blu reader close', this.blu_reader)
      // process.exit(0)
    })
